...
\begin{itemize}
	...
	\item[A2.3] ~\\
	      ...
	      Predictors should have a consequence on the dependent output variable $y$.
	      If that's not the case for $x_3$, then it does not provide any additional information to the model
	      and it is not useful for the prediction, bringing up noise problems and cause overfitting.

	      %   The new regressor addition added a random variable independent from the dependent output variable $y$.
	      %   The model complexity is higher and the model family structure has changed.
	      %   The new regressor could capture the noise patterns in the data making worse predictions.
	      %   In windows:
	      %   \begin{itemize}
	      %       \item[a.] the train error could increase due to multicollinearity and noise
	      %       \item[b.] the test error could increase due to multicollinearity and noise
	      %       \item[c.] the coefficients could be unstable and not unique
	      %             as the model is learning noise patterns from the random variable
	      %   \end{itemize}
